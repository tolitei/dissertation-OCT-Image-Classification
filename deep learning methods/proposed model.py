# -*- coding: utf-8 -*-
"""
Created on Thu Jun 29 23:32:24 2023
...
from keras.layers import GlobalAveragePooling2D, Dense, Dropout, BatchNormalization,Input, Flatten, Conv2D,MaxPooling2D
...
def preprocessing(img):
        ...
        top_region = np.array(img)[:80,:]
    
        bottom_region = np.array(img)[190:,:]
        
        # Apply thresholding to the top and bottom regions
        ...
        img_arr[:80,:] = top_region_thresh
        img_arr[190:,:] = bottom_region_thresh
        
        # Preprocessing 2: Apply Non-Local Means Filtering
...
def last_layer(bottom_model, num_classes): 
...
for fold, (train_idx, val_idx) in enumerate(splits):
    # Create the new training and Validation DataFrame
        ...
        for batch_size in BATCH_SIZES:
            ...
            #callbacks
            checkpoint = ModelCheckpoint(filepath=f"/home/tteixeira/CNN/Results/model.h5", monitor='val_fbeta', mode='max', verbose=1,save_best_only=True)   
            min_delta=0.000001
            ...
            bottom_model=create_model(input_shape)
            FC_Head= last_layer(bottom_model,num_classes)
            model=Model(bottom_model.input, outputs= FC_Head)
            model.summary()
            model.compile(optimizer=keras.optimizers.Adam(learning_rate=0.001),
                          loss='categorical_crossentropy', #- ver isto
                          metrics=all_metrics)
            # Create the generators for the current split
