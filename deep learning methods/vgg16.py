...
def preprocessing(img):
        ...
        ####################### Preprocessing 1 - Apply thresholding to top and bottom regions##################
        thresh_value = 240
...
# Create a new model without the last X layers
vgg = Sequential()
for layer in pretrained_model.layers[:-4]:
    vgg.add(layer)

def vgg16_new_layers(bottom_model, num_classes): 
...
for fold, (train_idx, val_idx) in enumerate(splits):
    ...
    for batch_size in BATCH_SIZES:
            ...
            FC_Head= vgg16_new_layers(pretrained_model,num_classes)
